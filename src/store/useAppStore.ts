...
function dbToIntern(row: DbIntern): Intern {
  return {
    ...
    journalism: (row as any).journalism || '',
    bikeProgram: (row as any).bike_program || '',
    itCertification: (row as any).it_certification || '',
...
export const useAppStore = create<AppState>()((set, get) => ({
  ...
  uploadExcelInterns: async (parsed: Intern[]) => {
    ...
    const inserts = parsed.map(i => ({
      ...
      journalism: i.journalism, bike_program: i.bikeProgram, it_certification: i.itCertification,
